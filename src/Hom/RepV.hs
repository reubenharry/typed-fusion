...
repVToV :: forall ts. KnownFTrees ts => RepV ts -> ToVRep ts
repVToV = go (fTreesSing @ts)
  where
    go :: forall ts'. SFTrees ts' -> RepV ts' -> ToVRep ts'
    go (SFTreesCons _ SFTreesNil) (RCons v RNil) = v
    go (SFTreesCons _ sRest@(SFTreesCons {})) (RCons v rest) =
      (v, go sRest rest)
    go _ _ = error "repVToV: expected nonempty KnownFTrees"

-- | Inverse of 'repVToV'.
vToRepV :: forall ts. KnownFTrees ts => ToVRep ts -> RepV ts
vToRepV = go (fTreesSing @ts)
  where
    go :: forall ts'. SFTrees ts' -> ToVRep ts' -> RepV ts'
    go (SFTreesCons (_ :: SFTree t) SFTreesNil) v =
      RCons @t v RNil
    go (SFTreesCons (_ :: SFTree t) sRest@(SFTreesCons {})) (v, rest) =
      RCons @t v (go sRest rest)
    go _ _ = error "vToRepV: expected nonempty KnownFTrees"
...
type family FuseRepTermC (rs :: FTrees) (qs :: FTrees) :: Constraint where
  FuseRepTermC '[] _ = ()
...
type family FuseRepOneTermC (t1 :: FTree) (qs :: FTrees) :: Constraint where
  FuseRepOneTermC _ '[] = ()
...
fuseRepTerm rs qs =
  ...
  where
    ...
    fuseRepOneTerm _ SFTreesNil RNil = RNil
    fuseRepOneTerm v1 (SFTreesCons (_ :: SFTree t2) qRest) (RCons v2 rest) =
      appendRepV
...
scaleRepV
  ...
  -> RepV ts
  -> RepV ts
scaleRepV s = go (fTreesSing @ts)
  where
    go :: SFTrees ts' -> RepV ts' -> RepV ts'
...
approxRepV
  ...
  -> RepV ts
  -> Bool
approxRepV = go (fTreesSing @ts)
  where
    closeVec a b =
      let da = toArray a
          db = toArray b
       in VS.all (\z -> magnitude z < 1e-9) (VS.zipWith (-) da db)
    go :: SFTrees ts' -> RepV ts' -> RepV ts' -> Bool
    go SFTreesNil RNil RNil = True
    go (SFTreesCons t rest) (RCons a as) (RCons b bs) =
      case t of
...
class FilterTrivialC (ts :: FTrees) where
